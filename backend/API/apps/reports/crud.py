...
def generate_ticket_pdf(db):
    ...
    ticket_df = ticket_df[['idTransaction','idSession','idEmployee','ticketCost','startTime','Name','Surname','phoneNumber']]
    ticket_df = ticket_df.merge(employee_df, left_on='idEmployee', right_on='idEmployee')
    ticket_df = ticket_df[['idTransaction','idSession','ticketCost','startTime','Name_x','Surname_x','phoneNumber_x','Name_y','Surname_y','phoneNumber_y']]
    ...
    for obj in ticket_df.iterrows():
        ...
        res = session_info_crud.get_film_on_session([('idSession',obj[1].idSession)], db)
        margin = 0
        for film in res:
